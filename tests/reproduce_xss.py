...
def test_xss_in_segment_name():
    ...
    for md in at.markdown:
        if malicious_name in md.value:
            ...
            print(
                f"VULNERABILITY CONFIRMED: Malicious string found unescaped in markdown: {md.value}"
            )
    ...
    if not found_vulnerable:
        print(
            "Vulnerability not found in markdown (it might already be escaped or not rendered)."
        )
